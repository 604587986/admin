...
<template>
  <div id="qingjiaxiangqing" class="detail-wrapper">
    <!-- 面包屑 -->
    ...
    <el-card class="box-card">
      ...
      <div class="body">
        ...
        <p><label>请假天数:</label><span>{{dataList.year}}</span></p>
        <p>
          <label>请假类型:</label>
          <span v-show="dataList.request_type==1">事假
            </span>
          <span v-show="dataList.request_type==2">病假
            </span>
          <span v-show="dataList.request_type==3">补假
            </span>
        </p>
        <p><label>开始时间:</label><span>{{dataList.start_time}}</span></p>
        <p><label>结束时间:</label><span>{{dataList.end_time}}</span></p>
        <p><label>课程名称:</label><span>{{dataList.curriculum}}</span></p>
        <p><label>所在班级:</label><span>{{dataList.squad}}</span></p>
